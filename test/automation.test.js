...
test("Ashby contract location can fall back from a city to its country", async () => {
  const result = await run(`<form>
    <div class="ashby-application-form-field-entry" data-field-path="location-id">
      ...
      <input role="combobox" aria-expanded="false" oninput="this.setAttribute('aria-expanded','true');choices.hidden=this.value!=='Bulgaria'">
      <div id="choices" role="listbox" hidden><div role="option" onclick="document.querySelector('[role=combobox]').value='Bulgaria';document.querySelector('[role=combobox]').setAttribute('aria-expanded','false');choices.hidden=true">Bulgaria</div></div>
    </div>
    <button type="submit">Submit Application</button>
  </form>`, { "location-id": "Sofia, Bulgaria" }, { ...profile,
    contact: { ...profile.contact, city: "Sofia", country: "Bulgaria", location: "Sofia, Bulgaria" }
  ...
  assert.equal(result.requirements[0].kind, "final_submission_approval");
  assert.equal(result.requirements[0].preview.filled
    .find((field) => field.key === "location-id").value, "Bulgaria");
});
