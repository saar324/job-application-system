...
test("Greenhouse email security code after Submit pauses without a receipt or retry", async () => {
  ...
  const html = `<form id="application" onsubmit="event.preventDefault();
      this.remove(); document.querySelector('#verification').hidden = false">
    <label>Email <input name="email" type="email" required></label>
    <button type="submit">Submit application</button>
  </form>
  <form id="verification" hidden>
  ...
  assert.match(result.requirements[0].message, /Do not restart or resubmit/);
