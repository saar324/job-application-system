...
test("user-requested exact domains are scoped and private DNS answers are rejected", async () => {
  const publicPolicy = createUrlPolicy({ WORKER_ALLOWED_DOMAINS: "greenhouse.io" },
    async () => [{ address: "8.8.8.8", family: 4 }]);
  assert.equal(publicPolicy.assertAllowed("https://careers.example.com/apply", ["careers.example.com"]).hostname,
    "careers.example.com");
  assert.throws(() => publicPolicy.assertAllowed("https://other.example.com", ["careers.example.com"]), /not allowed/);
