...
When an employer asks for an emailed security code after the authorized final click, the worker SHALL identify that requirement separately from an unknown submission outcome. It SHALL keep the durable final-action marker, request human review of any available employer verification path and reconciliation of the outcome, and SHALL NOT record a receipt or automatically click Submit again. It SHALL NOT imply that a closed worker browser session can be resumed. A retry request SHALL check the worker's durable final-action state before requeueing.
...
- **THEN** the application pauses as `submission_email_verification`, tells the owner the browser session ended and the employer outcome needs human reconciliation, and remains without a receipt and fenced against automatic retry
