...
6. Before asking for an application answer, run `node {baseDir}/scripts/find-prior-answer.js "Company" "Question"` and review its same-employer candidates from the application log. Also check the saved profile, resume, portfolio, and prior owner messages. Compare the meaning of each candidate; a high text-similarity score is a lead, not approval. Reuse an exact previously submitted answer when the owner has authorized that reuse, and store the current question and verbatim answer in `applicationAnswers` with `jobctl profile-update` so the worker can find it next time. If no clear, verified answer exists, ask the owner the question as written and do not submit while it is unresolved. If the employer prohibits AI-written answers, never draft or paraphrase those fields; enter only owner-provided wording or an exact prior submitted answer whose reuse the owner has authorized.
