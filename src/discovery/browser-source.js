...
export function extractSourcePage(html, pageUrl, sourceId, { residenceCountry } = {}) {
  ...
  const fallback = structured.length ? null : sourceId === "gisjobs"
    ? extractGisJob(html, pageUrl) : fallbackJob(html, pageUrl, sourceId,
      currentText, currentLocation, residenceCountry);
...
function fallbackJob(html, pageUrl, sourceId, currentText = visibleText(html), currentLocation,
  ...
  const location = currentLocation ?? explicitRemoteLocation(visible, residenceCountry);
  ...
  return { source: sourceId, title, company, description: visible.slice(0, 50_000),
    location: compatibleRemoteLocation(location, residenceCountry), remote: true,
    employmentType: employmentType(visible),
...
function compatibleRemoteLocation(location, residenceCountry) {
  return /\b(remote|distributed)\b/i.test(location) ? location
    : regionPattern(residenceCountry).test(location)
      ? `Remote, ${location}` : location;
}

function regionPattern(residenceCountry) {
  const country = String(residenceCountry ?? "").trim();
  const terms = ["EU", "EEA", "Europe", "European", "EMEA", "worldwide", "anywhere", "global",
    ...(country ? [escapePattern(country)] : [])];
...
function explicitRemoteLocation(text, residenceCountry) {
  const labelled = text.match(/(?:remote\s+location|location|eligible\s+locations?|work\s+from)\s*:?\s*([^\n]{0,140})/i)?.[1];
  const region = regionPattern(residenceCountry).exec(String(labelled ?? ""))?.[1]
    ?? new RegExp(`\\b(?:remote|distributed)[^\\n]{0,80}${regionPattern(residenceCountry).source}`, "i")
      .exec(text)?.[1]
    ?? new RegExp(`${regionPattern(residenceCountry).source}[^\\n]{0,80}\\b(?:remote|distributed)\\b`, "i")
      .exec(text)?.[1];
